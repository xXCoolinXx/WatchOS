...
const byte PROGMEM calculator_icon[] {
  0x00, 0x00, 0x00,
  0x1f, 0xfe, 0x00,
  0x10, 0x02, 0x00,
  0x17, 0xfa, 0x00,
...
const byte PROGMEM SpaceInvaders_icon[]
{
  0x00, 0x00, 0x00,
  0x00, 0x00, 0x00,
  0x00, 0x00, 0x00,
...
void Clock::blinky(byte currentPlace) {
  ...
  if (current_place == currentPlace && set_Time == true) {
    display.setTextColor(YELLOW, WHITE);
  }
}
...
void Clock::Left() {
  if (!clockFullScreen) {
    //
    if (Set && !lastLeft && (timerAlarm == 1 || timerAlarm == 2) ) {
      menuSetPlace--;
      if (menuSetPlace == 255) {
        menuSetPlace = 2;
      }
...
void Clock::Right() {
  if (!clockFullScreen) {
    if (Set && !lastRight && (timerAlarm == 1 || timerAlarm == 2) ) {
      menuSetPlace++;
      if (menuSetPlace == 3) {
        menuSetPlace = 0;
      }
...
void alarm()
{
  ...
  for (byte thisNote = 0; thisNote < 18; thisNote++) {
    ...
    tone(8, pgm_read_word(&Alarm_Notes[thisNote]), noteDuration);
...
void Clock::Special() {
  if (!clockFullScreen) {
    if (set_Time == false) {
      ...
      if (ta == true && timerAlarm == 2 && lastSpecial == false) {
        if (Rect{0, 9, 17, 9} .includes(Cursor)) {
          Set = true;
          whichAlarm = 1;
        }
        if (Rect{0, 24, 17, 9} .includes(Cursor)) {
          Set = true;
          whichAlarm = 2;
        }
        if (Rect{0, 39, 17, 9} .includes(Cursor)) {
          Set = true;
          whichAlarm = 3;
        }
      }
    ...
    if (!lastSpecial) {
      ...
      if (Rect{display.width() - 60, 41, 7, 9} .includes(Cursor) && swOn) {
        if (startSW == 1) {
          startSW = 0;
        }
        else if (startSW == 0) {
  ...
  if (Rect{(display.width() - 27) / 2, 31, 9, 7} .includes(Cursor) && !lastSpecial) {
    display.fillRect(0, 0, 96, 50, BLACK);
    button--;
    if (button == 255) {
      button = 2;
    }
  ...
  if (!clockFullScreen) {
    ...
    { //Display Time
      ...
      */
      ...
      blinky(2);
      switch (tm.Wday) {
        case 1: display.print("Su"); break;
        case 2: display.print("Mo"); break;
        case 3: display.print("Tu"); break;
        case 4: display.print("We"); break;
        case 5: display.print("Th"); break;
        case 6: display.print("Fr"); break;
        case 7: display.print("Sa"); break;
        default:
...
#define space (display.width() - 80) / 3
      ...
      if (button == 0) {
        ...
        display.drawBitmap(apps[0].x + 1, apps[0].y + 1, flashlight_icon, 18, 18, WHITE);
        display.drawBitmap(apps[1].x + 1, apps[1].y + 1, pong_icon, 18, 18, WHITE);
        display.drawBitmap(apps[2].x + 1, apps[2].y + 1, maze_icon, 18, 18, WHITE);
        display.drawBitmap(apps[3].x + 1, apps[3].y + 1, SpaceInvaders_icon, 18, 18, WHITE);
      }
      if (button == 1) {
        display.drawBitmap(apps[0].x + 1, apps[0].y + 1, calculator_icon, 18, 18, WHITE);
        //display.drawBitmap(apps[1].x + 1, apps[1].y + 1, paint_icon, 18, 18, WHITE);
    ...
    if (ta == true) {
      ...
      if (timerAlarm == 2) {
        display.setCursor(display.width() / 2 - 14, 0);
        display.print("Alarm");
        display.fillRoundRect(0, 9, 17, 9, 1, WHITE);
        display.setTextColor(BLACK);
        ...
        print(&alarm1);
        display.fillRoundRect(0, 24, 17, 9, 1, WHITE);
        display.setTextColor(BLACK);
        ...
        print(&alarm2);
        display.fillRoundRect(0, 39, 17, 9, 1, WHITE);
        display.setTextColor(BLACK);
  ...
  //Clear the display
  display.fillRect(0, 54, 96, 64, BLACK);
  display.drawBitmap(Cursor.x, Cursor.y, cursOR, 4, 4, BLACK);
  if (sw.Minute > 0 || sw.Second > 0 || sw.Hour > 0) {
    //display.clearScreen();
    display.fillRect(display.width() - 60, 41, 60, 9, BLACK);
  }
  if (timerAlarm > 0) {
    display.clearScreen();
  }
  display.fillRect(15, 42, 19, 7, BLACK);
...
void Clock::checkAlarm(alarmTime* alarm_) {
  if (tm.Hour == alarm_->Hour && tm.Minute == alarm_->Minute && !alarm_->active && alarm_->on)
  {
    alarm();
    alarm_->active = true;
  }
  else if (tm.Hour != alarm_->Hour && tm.Minute != alarm_->Minute && alarm_->on) {
    alarm_->active = false;
  }
}
...
Clock::Clock() {
  ...
  alarm1.whichAlarm = 1;
  alarm1.Hour = EEPROM.read(3);
  alarm1.Minute = EEPROM.read(4);
  alarm1.on = EEPROM.read(5);

  alarm2.whichAlarm = 2;
  alarm2.Hour = EEPROM.read(6);
  alarm2.Minute = EEPROM.read(7);
  alarm2.on = EEPROM.read(8);

  alarm3.whichAlarm = 3;
  alarm3.Hour = EEPROM.read(9);
  alarm3.Minute = EEPROM.read(10);
  alarm3.on = EEPROM.read(11);
...
void Clock::Toggle() {
  ...
  if (ta && timerAlarm == 0 && !lastToggle && !Set) {
    ...
    if (timerAlarm == 2) {
      whichAlarm = 0;
      EEPROM.write(3, alarm1.Hour);
      EEPROM.write(4, alarm1.Minute);
      EEPROM.write(5, alarm1.on);

      EEPROM.write(6, alarm2.Hour);
      EEPROM.write(7, alarm2.Minute);
      EEPROM.write(8, alarm2.on);

      EEPROM.write(9, alarm3.Hour);
      EEPROM.write(10, alarm3.Minute);
      EEPROM.write(11, alarm3.on);
    }
  }
  if (set_Time == true) {
    set_Time = false;
...
void Clock::stopwatch() {
  ...
  if (sw.Second != 0 || sw.Minute != 0 || sw.Hour != 0) {
    sleeper.resetSleep();
    display.setTextScale(1);
    display.setCursor(display.width() - 50, 42);
    print2(sw.Hour);
    display.print(":");
    print2(sw.Minute);
    display.print(":");
    ...
    display.fillRect(display.width() - 60, 41, 7, 9, WHITE);
    display.setCursor(display.width() - 59, 42);
    display.setTextColor(BLACK, WHITE);
    display.println("S");
    display.setTextColor(WHITE);
...
void Clock::highlightAlarm(alarmTime* Alarm, byte place) {
  if (Set == true && timerAlarm == 2 && Alarm->whichAlarm == whichAlarm && menuSetPlace == place) {
    display.setTextColor(BLACK, WHITE);
  }
}
void Clock::print(alarmTime* Alarm) {
  highlightAlarm(Alarm, 0);
  if (Alarm->Hour <= 12) {
    print2(Alarm->Hour);
  }
  else {
    print2(Alarm->Hour - 12);
  }
  display.setTextColor(WHITE);
  display.print(":");
  highlightAlarm(Alarm, 1);
  print2(Alarm->Minute);
  display.setTextColor(WHITE);
  display.print(" ");
  if (Alarm->Hour > 12) {
